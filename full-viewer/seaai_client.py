...
class SeaAIWebSocketClient:
    def __init__(self, settings: Settings, state: PersistentState, initial_ws_url: str) -> None:
        self.settings = settings
        self.state = state
        self._task: asyncio.Task[None] | None = None
        self._ws_url = _normalize_ws_url(initial_ws_url)
    ...
    async def _run_forever(self) -> None:
        if connect is None:
            await self.state.set_connection_status(False, "Missing dependency: websockets")
            return
        ...
        while True:
            ...
            try:
                ...
                async with connect(self._ws_url, ping_interval=20, ping_timeout=20) as socket:
                    self._active_socket = socket
                    await self.state.set_connection_status(True)
                    async for raw_message in socket:
                        payload = raw_message.decode("utf-8", errors="ignore") if isinstance(raw_message, bytes) else str(raw_message)
                        safe_payload = _redact_large_payloads(payload)
                        try:
                            parsed = parse_seaai_message(payload)
                        except ValueError as exc:
                            logger.warning("Upstream websocket parse failure: %s | payload=%s", exc, safe_payload)
                ...
                logger.warning("Upstream websocket connection failed for %s: %s", self._ws_url, exc)
                await self.state.set_connection_status(False, str(exc))
                await self._wait_for_retry(reconnect_delay)
            else:
                await self.state.set_connection_status(False, "Upstream websocket disconnected")
                await self._wait_for_retry(reconnect_delay)
            finally:
                self._active_socket = None
